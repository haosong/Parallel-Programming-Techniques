...
int main(int argc, char **argv) {
    ...
    for (int run = 3; run < 4; run++) {
        ...
        if (rank == 0) {
            ...
            MPI_Waitall(size - 1, sendRequest, status);
            ...
            MPI_Waitall(2 * size - 2, sendRequest, status);
            ...
            MPI_Waitall(size - 1, recvRequest, status);
            ...
            MPI_Irecv(B, BLOCK_LEN(colIndex, blockSize), MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, &recvRequest[2]);
            MPI_Waitall(3, recvRequest, status);

            for (int i = 1; i < size; i++) {
                int nextColIndex;
                MPI_Recv(&nextColIndex, 1, MPI_INT, rank - 1, 1, MPI_COMM_WORLD, &recvRequest[0]);
                MPI_Recv(nextB, BLOCK_LEN(nextColIndex, blockSize), MPI_DOUBLE, rank - 1, 1, MPI_COMM_WORLD, &recvRequest[1]);
                MPI_Send(&colIndex, 1, MPI_INT, (rank + 1) % size, 1, MPI_COMM_WORLD, &sendRequest[0]);
                MPI_Send(B, BLOCK_LEN(colIndex, blockSize), MPI_DOUBLE, (rank + 1) % size, 1, MPI_COMM_WORLD, &sendRequest[1]);
